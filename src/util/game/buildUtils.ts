...
export const VERSION_MAP: {
  ...
  7423456: { version: "10.40", netcl: "++Fortnite+Release-10.40-CL-7423456-Windows" },
  12905909: { version: "12.41", netcl: "++Fortnite+Release-12.41-CL-12905909-Windows" },
  6932145: { version: "9.10", netcl: "++Fortnite+Release-9.10-CL-6932145-Windows" },
};
...
export const parseVersionInfo = (patternHexCheck: string[]) => {
  ...
  for (const str of patternHexCheck) {
    ...
    // Check VERSION_MAP for ANY build with this CL (not just Live/Cert)
    if (VERSION_MAP[cl]) {
  ...
  // Fallback: try to extract version from any string containing a version pattern
  for (const str of patternHexCheck) {
    ...
    if (simpleMatch) {
      ...
      return {
        version,
        netcl: `++Fortnite+Release-${version}-CL-Unknown-Windows`,
      };
    }
  }
